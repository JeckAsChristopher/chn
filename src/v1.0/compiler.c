...
static void emit_const(Compiler *C, Value val, int line){
    /* use fast single-byte opcodes for 0 and 1 */
...
static void emit_break(Compiler *C, int line){
    ...
    if(f->break_count>=MAX_BREAKS){ error_compile(line, 0, 0,"too many breaks"); C->had_error=true; return; }
...
static int global_define(Compiler *C, const char *name, bool is_let, int line){
    ...
    if(ex>=0){
        if(is_let&&!C->globals[ex].is_let){ error_compile(line, 0, 0,"cannot redeclare 'var %s' as 'let'",name); C->had_error=true; return ex; }
        if(!is_let&&C->globals[ex].is_let){ error_compile(line, 0, 0,"cannot redeclare 'let %s' as 'var'",name); C->had_error=true; return ex; }
        C->globals[ex].defined=false; return ex;
    }
    if(C->global_count>=MAX_VARIABLES){ error_compile(line, 0, 0,"too many globals"); C->had_error=true; return 0; }
...
static int local_define(Compiler *C, const char *name, bool is_let, int line){
    if(C->local_count>=MAX_LOCALS){ error_compile(line, 0, 0,"too many locals"); C->had_error=true; return 0; }
    ...
    /* record local name in co_varnames for debugger */
    Chunk *ch=C->current_chunk;
...
static void suggest_variable(Compiler *C, const char *name, int line){
    ...
    if(m) error_compile(line, 0, 0,"undefined variable '%s' -- did you mean '%s'?",name,m);
    else  error_compile(line, 0, 0,"undefined variable '%s'",name);
}
static void suggest_function(Compiler *C, const char *name, int line){
    ...
    int dist; const char *m=best_match(name,(const char**)cands,nc,&dist);
    if(m) error_compile(line, 0, 0,"undefined function '%s' -- did you mean '%s'?",name,m);
    else  error_compile(line, 0, 0,"undefined function '%s'",name);
}
...
static FuncRef resolve_function(Compiler *C, const char *name, int line){
    ...
    for(int i=0;i<C->func_count;i++){
        if(!strcmp(C->functions[i]->name,name)){
            ...
            if(callee->visibility==VIS_PROTECTED&&!func_can_access(C->current_func,callee)){
                error_compile(line, 0, 0,"protected function '%s' not accessible here",name);
    ...
    for(int i=0;i<C->import_count;i++){
        ...
        /* 1.0: intra-bundle pass - skip visibility/export checks so that
         * non-exported functions from other units in the same -oc bundle
         * are callable freely. Outside normal compilation these still block. */
        if(!C->is_bundle_pass){
            if(f->visibility==VIS_PRIVATE){ error_compile(line, 0, 0,"function '%s' is private",name); C->had_error=true; return ref; }
            if(!f->exported){ error_compile(line, 0, 0,"function '%s' is not exported",name); C->had_error=true; return ref; }
        }
...
static void emit_store(Compiler *C, const char *name, int line, bool allow_def){
    if(C->in_function){
        ...
        if(slot>=0){
            ...
            /* If function has captures, sync local value to its global slot.
             * SET_VAR uses TOP (no pop), GET_LOCAL pushes a copy.           */
            if(C->current_func && C->current_func->has_captures){
                ...
                if(gidx>=0){
                    ...
                    emit_op(C,OP_POP,line);  /* pop the GET_LOCAL copy; SET_VAR left it */
                }
            }
            return;
        }
...
static void compile_expr(Compiler *C, ASTNode *node);  /* fwd */

/* Returns true if the AST subtree contains any nested function or lambda decl. */
static bool has_nested_func(ASTNode *n){
    if(!n) return false;
    if(n->kind==NODE_FUNC_DECL||n->kind==NODE_LAMBDA) return true;
    /* Recurse into common container nodes */
    switch(n->kind){
        case NODE_BLOCK:
        case NODE_PROGRAM:
    ...
    while(*p){
        if(*p=='{'&&*(p+1)!='{'){
            ...
            if(*end=='\0'&&end!=expr){
                ...
                /* Parse the expression snippet through the real parser
                 * so that method calls, field access, arithmetic etc. work. */
...
static void compile_expr(Compiler *C, ASTNode *node){
    ...
    case NODE_DICT_LITERAL:{
        ...
        for(int i=0;i<node->dict_lit.count;i++){
            /* v3.3 fix: OP_DICT_SET pops (dict, key, val) and pushes dict back.
             * Without OP_POP each iteration leaves an extra dict reference on the
             * stack -- after N pairs the stack holds N+1 refs causing stack
             * corruption for every dict literal with >0 entries. */
            emit_op(C,OP_DUP,line);        /* dup dict ref for DICT_SET */
            compile_expr(C,node->dict_lit.keys[i]);
            compile_expr(C,node->dict_lit.values[i]);
            emit_op(C,OP_DICT_SET,line);   /* pops dup+key+val, pushes dict */
            emit_op(C,OP_POP,line);        /* discard returned ref; original stays */
        }
        break;
    }
    ...
    case NODE_COMPOUND_ASSIGN:{
        ...
        switch(node->compound_assign.op){
            ...
            case TK_STARSTAR_ASSIGN: emit_op(C,OP_POW,line); break; /* v3.6 */
    ...
    /* x++ -> load x, push 1, add, store x (new val on TOS; EXPR_STMT pops)
     * Semantics: as an expression, POST_INC evaluates to the OLD value.
     * As a statement (most common use), the result is discarded anyway.
     * We emit: load, load, +1, store, pop-new -> old stays for expr context.  */
    case NODE_POST_INC:{
        emit_load(C,node->ident.name,line);   /* old value on TOS (the result)  */
        emit_load(C,node->ident.name,line);   /* second copy to compute new val  */
        emit_op(C,OP_CONST_1,line);
        emit_op(C,OP_ADD,line);
        emit_store(C,node->ident.name,line,false);
        emit_op(C,OP_POP,line);               /* pop new value; old stays on TOS */
        break;
    }
    /* ++x -> new value on TOS */
    case NODE_PRE_INC:{
    ...
    /* val in container -- handled via OP_IN in vm */
    case NODE_TERNARY:{
    ...
    case NODE_TYPEOF:{
        /* evaluate typeof at compile time when operand type is known */
    ...
    case NODE_BINARY:{
        /* v3.1 constant-fold: already handled by fold_all pre-pass at opt>=1.
         * This inline guard handles the -O0 case where fold_all was skipped. */
        if(C->opt_level>=1 &&
        ...
        switch(node->binary.op){
            ...
            case TK_IN:       emit_op(C,OP_IN,line); break; /* v3.6 */
    ...
    case NODE_CALL:{
        /* first try to resolve as a named (statically declared) function.
         * If not found, try loading it as a variable holding a lambda/func value.
         * This lets   var f = x -> x+1; f(5)   work naturally.              */
        ...
        if(is_named_func){
            ...
            /* Dynamic call - name is a variable holding a func value */
            bool found_var = (C->in_function && local_find(C,node->call.name)>=0)
                           || global_find(C,node->call.name)>=0;
            if(!found_var){
                /* Unknown - let suggest_function give the best error */
    ...
    case NODE_METHOD_CALL:{
        /* 1.0: check if object is a module alias - pkg.func(args) */
           ...
           C->alias_count > 0)
        {
            ...
            for(int ai = 0; ai < C->alias_count; ai++){
                ...
                /* Found alias - resolve function from its import slice */
                ...
                if(!found_fn){
                    error_compile(line, 0, 0,"'%s' has no function '%s'", obj_name, fn_name);
                    C->had_error=true; return;
                }
                if(node->method_call.arg_count != found_fn->arity){
                    error_compile(line, 0, 0,"'%s.%s' expects %d arg(s) but got %d",
                        obj_name, fn_name, found_fn->arity, node->method_call.arg_count);
                    C->had_error=true; return;
                }
                /* Emit as a static function call - same as NODE_CALL */
            ...
            /* obj_name is not an alias - fall through to normal method dispatch */
        }
        ...
        if(mid==METHOD_UNKNOWN){
            ...
            if(m) error_compile(line, 0, 0,"unknown method '%s' -- did you mean '%s'?",node->method_call.method,m);
            else  error_compile(line, 0, 0,"unknown method '%s'",node->method_call.method);
            C->had_error=true; return;
        }
    ...
    case NODE_THROW:{
        ...
        emit_op(C,OP_THROW,line); break;
    }


    /* null -- compiles identically to nil (same runtime value VAL_NIL) */
    case NODE_NULL:
        emit_op(C,OP_NIL,line); break;

    /* await expr -- emits OP_AWAIT which is currently an identity op.
     * When a native async function returns a "promise" dict, OP_AWAIT
     * will eventually spin an event loop. For now it is a no-op so
     * existing async/await code compiles and runs correctly. */
    case NODE_AWAIT:
        compile_expr(C,node->await_expr.expr);
        emit_op(C,OP_AWAIT,line); break;

    /* expr.field -- compiled as a string-key dict lookup:
     *   push object, push string-key, OP_DICT_GET
     * Works seamlessly with struct instances (which are dicts). */
    ...
    /* expr.field = val  ->  dict key assignment.
     * OP_DICT_SET pops (dict, key, val) and pushes dict back.
     * We leave that dict on stack -- the enclosing EXPR_STMT pops it.    */
    case NODE_FIELD_SET:{
    ...
    /* --  lambda expression -----------------------------------------
     * Compiles just like NODE_FUNC_DECL but:
     *   * Gets a compiler-generated unique name  __lambda_N
     *   * Expression-body forms wrap the expr in an implicit return
     *   * The FunctionObject is pushed onto the stack as a FUNC_VAL
     *     (so it can be stored in a var, passed, or called immediately)  */
    case NODE_LAMBDA:{
        /* Generate unique name */
        char lname[64];
        snprintf(lname,sizeof(lname),"__lambda_%d", ++C->lambda_count);

        /* Create FunctionObject (not in prescan, created fresh here) */
        ...
        /* Promote outer locals to globals so this lambda can capture them */
        if(saved_if){
        ...
        if(node->lambda.is_expr_body){
            ...
            /* Block form: body is a NODE_BLOCK - explicit returns */
        ...
        /* Push the function object as a value onto the stack */
        emit_const(C, FUNC_VAL(f), line);
        break;
    }

    /* dynamic call  callee_expr(arg1, arg2, ...)
     * The callee can be any expression that evaluates to a function value:
     *   (x -> x+1)(5)          -- immediately invoked lambda
     *   var f = x -> x*2; f(3) -- stored lambda via ident
     *   fns[0](arg)            -- lambda in array                          */
    case NODE_CALL_EXPR:{
        /* Evaluate callee, then each argument, then OP_CALL argc */
    ...
    /* Statement nodes that are unreachable from compile_expr - silence -Wswitch */
...
static void compile_stmt(Compiler *C, ASTNode *node){
    ...
    case NODE_VAR_DECL:{
        ...
        if(C->in_function){
            ...
            C->locals[slot].defined=true;
            /* If enclosing func has nested closures, ALSO define as global so
             * nested funcs can capture the variable by name.
             * DUP the value (SET_LOCAL leaves it on TOS), then DEF_VAR pops.  */
            if(C->current_func && C->current_func->has_captures){
    ...
    case NODE_DO_WHILE:{
        ...
        int jt=emit_jump(C,OP_JUMP_IF_TRUE,line);
        emit_op(C,OP_POP,line);  /* condition was false: fall through to break_target */
        int jend=emit_jump(C,OP_JUMP,line); /* skip back-edge */
        patch_jump(C,jt);
        emit_op(C,OP_POP,line);  /* pop true condition before looping */
        emit_loop(C,loop_start,line);
        patch_jump(C,jend);
        pop_ctx_patch_breaks(C); break;
    }
    ...
    case NODE_BLOCK:{
        /* v3.0 optimisation: dead-code elimination -- stop emitting bytecode
         * for statements that follow an unconditional transfer (return/break/
         * continue). Emit a compile error for each unreachable statement.    */
        bool dead=false;
        for(int i=0;i<node->block.count;i++){
            ASTNode *s=node->block.stmts[i];
            if(dead){
                /* skip empty expression statements (e.g. stray newlines) */
    ...
    case NODE_RETURN:{
        ...
        if(node->ret.value){
            /* tail-call optimisation -- return func(args) -> OP_CALL_TAIL
             *  skip TCO if the callee name is a variable (lambda stored in var)
             *       to avoid resolve_function erroring on a valid dynamic call.   */
            ASTNode *rv=node->ret.value;
            if(C->opt_level>=2 && rv->kind==NODE_CALL){
                /* Only attempt TCO for statically declared functions */
    ...
    case NODE_FUNC_DECL:{
        ...
        /* Scan body for nested functions/lambdas; if present, vars need capture */
        ...
        /* Save outer globals count so we can add outer locals as temporaries */
        int saved_gc = C->global_count;
        Symbol saved_globals[MAX_VARIABLES];
        memcpy(saved_globals, C->globals, sizeof(saved_globals));

        /* Promote outer locals into the global table so nested function body
         * can resolve them via OP_GET_VAR / OP_SET_VAR (captured globals).
         * This is a conservative closure approximation: captured locals are
         * shared by name with the enclosing scope via globals.             */
        if(saved_if) {
            for(int ci = 0; ci < saved_lc; ci++){
                bool already = false;
                for(int gi = 0; gi < C->global_count; gi++){
        ...
        for(int i=0;i<f->arity;i++){
            int slot=local_define(C,f->params[i],false,line);
            C->locals[slot].defined=true;
            /* If has_captures: sync param to global so nested funcs can read it */
            if(f->has_captures){
        ...
        memcpy(C->locals,saved_locs,sizeof(saved_locs));
        memcpy(C->ctx_stack,saved_ctx,sizeof(saved_ctx));
        /* Restore globals but keep any that were freshly defined inside nested func */
        for(int gi = saved_gc; gi < C->global_count; gi++){
            bool outer = false;
            for(int ci = 0; ci < saved_lc; ci++){
                if(!strcmp(C->globals[gi].name, saved_locs[ci].name)){ outer=true; break; }
            }
            if(!outer){ /* truly new global defined inside nested func: keep it */
    ...
    case NODE_IMPORT:{
        /* Record import slice boundaries for alias tracking (v4.3) */
        ...
        if(node->import.is_lib){
            ...
            if(lib_handler){
                ...
                if(!import_ok){ error_compile(line, 0, 0,"failed to load CCO bundle '%s'",node->import.path); C->had_error=true; }
            } else { error_compile(line, 0, 0,"import not supported"); C->had_error=true; }
        } else {
            ...
            if(handler){
                ...
                if(!import_ok){ error_compile(line, 0, 0,"failed to import '%s'",node->import.path); C->had_error=true; }
            } else { error_compile(line, 0, 0,"import not supported"); C->had_error=true; }
        }

        /* 1.0: if  as <alias>  was specified, record the alias -> import slice */
        ...
        for(int i=0;i<C->func_count;i++){
            if(!strcmp(C->functions[i]->name,node->export_node.name)){
                if(C->functions[i]->visibility==VIS_PRIVATE){
                    error_compile(line, 0, 0,"cannot export private function '%s'",node->export_node.name);
                    C->had_error=true;
                } else { C->functions[i]->exported=true; }
                found=true; break;
            }
        }
        if(!found){ error_compile(line, 0, 0,"cannot export '%s': no such function",node->export_node.name); C->had_error=true; }
        break;
    }
    ...
    /* -- v3.5 new statement nodes ----------------------------------------
     *
     * struct Name { x: 0, y: 0 }
     *
     * Compiles into a synthetic factory function named after the struct:
     *
     *   func Name(__x, __y) {         <- params named __field
     *     dict d = {}
     *     d.x = __x                   <- OP_DICT_SET each field
     *     d.y = __y
     *     return d
     *   }
     *
     * Calling Name(1, 2) returns {"x":1, "y":2}.
     * Fields with defaults become default-arg patterns: when the caller
     * passes nil (or null) for a param, the default is substituted.
     *
     * Implementation: we build a FunctionObject by hand and emit bytecode
     * into its chunk directly, then register it like a normal func.
     */
    case NODE_STRUCT_DECL:{
        int fc = node->struct_decl.field_count;
        const char *sname = node->struct_decl.name;

        /* Reuse the stub registered by prescan (same pattern as NODE_FUNC_DECL).
         * If prescan didn't run (e.g. single-stmt compile), create fresh. */
        FunctionObject *f = NULL;
        for(int i=0;i<C->func_count;i++){
        ...
        if(!f){
            ...
            /* Prescan stub already has correct arity/params; just update metadata */
            f->visibility = VIS_PUBLIC;
            strncpy(f->source_file, C->source_file, sizeof(f->source_file)-1);
        }

        /* Save compiler state and switch to the factory chunk */
        Chunk          *saved_chunk = C->current_chunk;
        Local           saved_locs[MAX_LOCALS]; memcpy(saved_locs,C->locals,sizeof(saved_locs));
        int             saved_lc    = C->local_count;
        bool            saved_if    = C->in_function;
        ...
        for(int i=0;i<fc;i++){
            ...
            /* Value: load param; if null use default */
            int param_slot = i; /* params are slots 0..fc-1 */
            ...
            /* OP_DICT_SET pops (dict, key, val) and pushes dict */
            emit_op(C, OP_DICT_SET, line);
            /* Store back into dict_slot (SET_VAR leaves TOS; POP it) */
    ...
    /* null as a statement (rare but valid: just emits nil and pops) */
    case NODE_NULL:
        emit_op(C,OP_NIL,line);
        emit_op(C,OP_POP,line); break;
...
static void prescan_stmt(Compiler *C, ASTNode *node){
    ...
    switch(node->kind){
        ...
        case NODE_EXPORT:
            ...
            break;
        /* pre-register struct factory functions so they can be called
         * before their declaration site (same forward-decl semantics as funcs) */
        case NODE_STRUCT_DECL:{
            const char *sname=node->struct_decl.name;
            bool already=false;
...
/* -- v3.1 multi-level constant folding -------------------------------------
 * Recursively collapses chains of numeric binary expressions into a single
 * NODE_NUMBER before any bytecode is emitted.
 * e.g.  10 * 10 + 7  ->  100 + 7  ->  107  (one OP_CONST instead of three ops)
/*
 * Returns true if the node was fully folded into a compile-time constant.   */
static bool ast_fold_constants(ASTNode *node, int opt_level){
    if(!node || opt_level < 1) return false;
    if(node->kind==NODE_NUMBER) return true;  /* already a literal */
    ...
    if(node->kind==NODE_BINARY){
        ...
        if(l && r &&
           ...
           node->binary.right->kind==NODE_NUMBER){
            ...
            if(ok){
                /* Mutate the node in-place into a NUMBER node */
                node->kind=NODE_NUMBER;
                node->num.value=result;
                /* The old child pointers leak here, but they are arena-style
                 * ASTNodes that will be freed by ast_free on the root anyway. */
                return true;
            }
        }
    }
...
/* Walk the whole program AST and fold every expression in-place */
static void fold_all(ASTNode *node, int opt_level);
static void fold_node(ASTNode *node, int opt_level){
    if(!node) return;
    ast_fold_constants(node, opt_level);  /* attempt fold on this node */
    fold_all(node, opt_level);            /* recurse into children */
}
static void fold_all(ASTNode *node, int opt_level){
...
/* -- v3.1 peephole optimizer -----------------------------------------------
 * Single pass over raw bytecode after codegen. Collapses common patterns:
 *   OP_CONST x + OP_POP      -> nothing    (push then immediately discard)
 *   OP_NOT   + OP_NOT        -> nothing    (double negation cancels)
 *   OP_NIL   + OP_POP        -> nothing
 *   OP_TRUE  + OP_POP        -> nothing
 *   OP_FALSE + OP_POP        -> nothing
 *
 * Jump threading: if OP_JUMP target is another OP_JUMP, forward to its target.
 */

/* Instruction widths (opcode byte + operand bytes) */
static int instr_width(uint8_t op){
    switch((OpCode)op){
        ...
        case OP_AWAIT:          /* identity, no operand */
        case OP_IN:             /* membership test, no operand */
        case OP_HALT:
            return 1;
        /* 3-byte (opcode + uint16) */
        ...
        case OP_METHOD_CALL:         /* opcode + mid:u8 + argc:u8 = 3 bytes */
        case OP_FOREACH_STEP:        /* opcode + end_offset:u16   = 3 bytes */
        case OP_PUSH_HANDLER:        /* opcode + catch_offset:u16 = 3 bytes */
        case OP_INC_LOCAL: case OP_DEC_LOCAL:
        case OP_PRINT: case OP_INPUT: case OP_PROMPT:
            return 3;
        /* v3.4 fix: these opcodes have NO operand -- VM reads 0 extra bytes.
         * Previously lumped into the 3-byte group, causing the peephole optimizer
         * and compute_stack_size to advance 2 bytes too far per occurrence,
         * reading into adjacent instructions and corrupting those passes on any
         * code that uses arrays or dicts. */
        ...
        case OP_FOREACH_INIT:   /* no operand; FOREACH_STEP is the 3-byte one */
        case OP_POP_HANDLER:    /* no operand; PUSH_HANDLER is the 3-byte one */
            return 1;
        /* 4-byte: OP_NATIVE (opcode + uint16 id + uint8 argc) */
        case OP_NATIVE:
            return 4;
        default:
            return 1;
...
static void peephole_optimize(Chunk *ch){
    ...
    while(i < len){
        ...
        /* Pattern: push-then-pop (CONST/NIL/TRUE/FALSE/CONST_0/CONST_1 + POP) */
        if(op2==OP_POP &&
           (op==OP_NIL||op==OP_TRUE||op==OP_FALSE||
            op==OP_CONST||op==OP_CONST_0||op==OP_CONST_1)){
            /* Replace both with NOPs -- use OP_GC_SAFEPOINT as a harmless NOP here
             * since it has no side effects on the stack. Actually safest: replace
             * with multiple NOP=OP_GC_SAFEPOINT bytes.
             * Simpler: just zero out the bytes and skip them as OP_GC_SAFEPOINT.  */
        ...
        /* Jump threading: JUMP -> JUMP forward to final destination */
        if((op==OP_JUMP||op==OP_JUMP_IF_FALSE||op==OP_JUMP_IF_TRUE||op==OP_JUMP_IF_NIL)
            ...
            while(visited < 32 && target < (uint16_t)len &&
                ...
                if(next_target == target) break; /* self-loop guard */
                target = next_target;
                visited++;
            }
...
/* -- v3.1 compile-time typeof ----------------------------------------------
 * Returns a static type-name string if the node's type is known at compile
/*
 * time (literal), NULL otherwise.                                            */
static const char *compiletime_typeof(ASTNode *node){
    if(!node) return NULL;
    switch(node->kind){
        ...
        case NODE_NIL:
        case NODE_NULL:     return "nil";   /* null typeof == "nil" */
        default:            return NULL;
    }
}

/* --  compute max stack depth by simulating the bytecode ------------
 * Single linear pass -- approximation (ignores branches) but correct for
/*
 * the stack-safe programs CHN produces.  Real depth = max(forward depth).  */
static void compute_stack_size(Chunk *ch){
    ...
    while(i<len){
        ...
        switch(op){
            ...
            case OP_SET_VAR: case OP_SET_LOCAL: delta= 0; break; /* peek, no pop */
            ...
            case OP_LSHIFT: case OP_RSHIFT:
            case OP_EQ: case OP_NEQ: case OP_LT: case OP_GT:
            case OP_LE: case OP_GE: delta=-1; break; /* pop 2 push 1 */
            ...
            case OP_CALL:{ int argc=(int)(code[i+1]|(code[i+2]<<8));
                           delta=-(argc); break; } /* pop func+args, push result */
            ...
            case OP_METHOD_CALL:{ /* v3.3 fix: encoding is [op][mid:u8][argc:u8] -- argc is code[i+2] only */
            ...
            case OP_FOREACH_INIT: delta=+2; break; /* push iter+index */
            case OP_FOREACH_STEP: delta=-1; break;
            case OP_INC_LOCAL: case OP_DEC_LOCAL: delta=0; break;
            case OP_NATIVE: delta=0; break; /* approximate */
            case OP_GC_SAFEPOINT: delta=0; break;
            case OP_AWAIT: delta=0; break;  /* identity, stack unchanged */
            case OP_IN:   delta=-1; break;  /* pops val+container, pushes bool */
            case OP_HALT: goto done;
            default: delta=0; break;
        }
        depth+=delta;
        if(depth<0) depth=0; /* guard against tracking errors */
        ...
        switch(op){
            ...
            case OP_FOREACH_STEP:   /* opcode + uint16 */
            case OP_PUSH_HANDLER:   /* opcode + uint16 */
            case OP_INC_LOCAL: case OP_DEC_LOCAL:
            case OP_PRINT: case OP_INPUT: case OP_PROMPT:
                i+=3; break;
            /* v3.4 fix: same correction as instr_width -- these are 1-byte (no operand) */
            case OP_ARRAY_NEW: case OP_ARRAY_PUSH:
            case OP_ARRAY_INDEX: case OP_ARRAY_SET:
            case OP_DICT_NEW: case OP_DICT_SET: case OP_DICT_GET:
    ...
    ch->stack_size=max_depth+4; /* +4 headroom */
}
void compiler_init(Compiler *C, const char *source_file){
    ...
    C->opt_level=2; /* default full optimization */
}
...
bool compiler_compile(Compiler *C, ASTNode *ast){
    ...
    /* multi-level constant folding pass over the full AST */
    ...
    if(ast->kind==NODE_PROGRAM){
        ...
        /* In program mode (entry main present), warn about top-level code that
         * sits outside entry main, imp, or function declarations.
         * v3.0 is lenient -- future 3.x releases will make this a hard error. */
        ...
        if(!C->had_error){
            if(entry_node){
                /* Program mode: compile imp + regular funcs first, then
                 * inline the entry body directly into top_chunk.            */
    ...
    /* compute max stack depth for each chunk */
    if(!C->had_error){
