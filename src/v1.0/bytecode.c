...
/* 1.0: global minify flag - default on, disabled by --no-minify */
bool g_minify = true;
...
/* Rename a symbol to _m{6hex}.  Stable: same input -> same output.
 * "<module>" and empty names are returned unchanged. */
...
/* Derive a per-chunk 16-bit seed from the (renamed) function name. Seed 0 is reserved for "no encoding"; bump to 1 if hash gives 0. */
...
/* LCG stream encode/decode (symmetric - applying twice restores original). Operates on raw bytecode bytes including operands. */
...
/* XOR-encode/decode a single string constant. key varies per string pool position so different strings have different keys. */
...
/* Returns number of bytes consumed AFTER the opcode byte itself.          */
static int opcode_extra(uint8_t op) {
    switch ((OpCode)op) {
        ...
        case OP_INPUT:
            return 3;
        /* 3-byte: u8 + u8 + u8 (nid_lo, nid_hi, argc) */
        case OP_NATIVE:
            return 3;
        /* 2-byte: u8 + u8 */
        case OP_METHOD_CALL:
...
/* Builds a compacted constant pool and a remapping table old->new index. Caller is responsible for freeing out_remap.                            */
typedef struct {
    Value   *vals;
    int      count;
    int      cap;
...
/* Dedup ch->constants[0..count] -> out_pool (unique values). out_remap[old_idx] = new_idx.  Returns false on OOM. */
...
/* Patch OP_CONST operands in a code copy using the dedup remap table. */
...
static void wb_const(WBuf *w, Value v){
    switch(v.type){
        case VAL_NUMBER:{
            ...
            if((double)as_int==n){
                /* Whole number - store as CTAG_INT for compactness */
                wb_u8(w,CTAG_INT);
                wb_i64(w,as_int);
            } else {
                wb_u8(w,CTAG_FLOAT);
...
static void wb_lnotab(WBuf *w, Chunk *ch){
    /* Convert RLE line runs into delta pairs: (byte_delta, line_delta)
     * Each entry covers the instructions from its offset to the next entry. */
...
/* When g_minify is true: renames symbol, deduplicates constants, XOR-encodes
 * strings and instruction stream, strips debug metadata.
 * The u16 minify_seed field (new in v4.4 / CHNO_VERSION 7) carries the key.
/*
 * Seed 0 means plaintext (--no-minify mode). VM never sees this field. */
static void wb_codeobj(WBuf *w, Chunk *ch, FunctionObject *fn) {
    char name_buf[64];
    const char *raw_name = fn ? fn->name : "<module>";
    uint16_t enc_seed = 0;

    /* Symbol renaming: ONLY apply to non-exported (internal) functions.
     * Exported names are public API - they must remain unchanged so that
     * importing modules can resolve them by original name.
     * Private/internal functions (not exported) are safe to rename.       */
    ...
    if (can_rename) {
        ...
        /* Still apply instruction/string encoding for exported fns when minifying */
    ...
    if (g_minify && ch->const_count > 0) {
        ...
        if (!did_dedup) {
            /* OOM fallback: use original pool without dedup */
    ...
    /* name */
    wb_cstr(w, name_buf);
    /* filename - always strip to empty when minifying */
    wb_cstr(w, g_minify ? "" : (fn ? fn->source_file : ""));
    /* 1.0: minify_seed (u16) - 0 = plain, nonzero = encoded */
    wb_u16(w, enc_seed);
    /* arity, flags, stack_size, visibility */
    ...
    for (int i = 0; i < pool.count; i++) {
        ...
        switch (v.type) {
            ...
            case VAL_STRING: {
                ...
                if (enc_seed && slen > 0) {
                    /* XOR-encode string bytes before writing */
            ...
            case VAL_NIL:
                ...
                break;
            case VAL_FUNCTION: {
                /* Rename FUNCREF target only if target is non-exported internal */
                wb_u8(w, CTAG_FUNCREF);
    ...
    if (code_buf) {
        /* minified: already XOR-encoded in code_buf */
    ...
    /* Cleanup dedup pool (only if we allocated it) */
...
static BCResult flush_chno(const char *path, uint16_t file_flags,
    ...
    /* Section directory: nsects - 12 bytes */
...
/*
 * bc_write_program - full program (.chno)
 */
BCResult bc_write_program(const char *path, Compiler *C){
    ...
    /* SECT_SYMBOLS - global variable names for debugger */
...
/*
 * bc_write_functions - module bundle (.function)
 */
...
/* enc_seed: 0 = plaintext, nonzero = decode strings with this seed. pool_idx: position in the pool (used to vary per-string XOR key).       */
...
/* Legacy rb_const (plaintext) - used by non-versioned readers */
...
/* 1.0: reads the minify_seed field (u16) inserted after filename. If seed != 0: XOR-decode string constants and instruction stream.        */
static FunctionObject *rb_codeobj(RBuf *r, Chunk *out_chunk){
    int nl=0; char *name=rb_str(r,&nl);
    int fl=0; char *file=rb_str(r,&fl);

    /* 1.0: minify_seed - present in CHNO v7+ files.
     * The caller (chno_open) passes the file version via the RBuf context.
     * We detect v7 by checking if we are reading from a v7 CHNO: the seed
     * field is always present in files we write (CHNO_VERSION 7). For
     * backward-compat we gate on a thread-local flag set by the caller. */
    extern uint16_t _rb_chno_ver; /* set by chno_open / cco_open callers */
    uint16_t enc_seed = 0;
    ...
    /* co_consts - decode strings if minified */
    ...
    /* co_code - XOR-decode if minified */
...
/* Thread-local CHNO version context for rb_codeobj */
uint16_t _rb_chno_ver = 0;


/*
 * File reader - parse CHNO file header + locate sections
 */
...
static BCResult chno_open(const char *path, CHNOFile *out){
    ...
    /* Section directory: nsects32 - 12 bytes */
    int ns=(int)nsects32; if(ns>16) ns=16;
    if(fseek(fp,0,SEEK_CUR)!=0){fclose(fp);return BC_ERR_IO;} /* keep position */
    SectDir sects[16];
    for(int i=0;i<ns;i++){
    ...
    /* Set global version context so rb_codeobj knows whether to read minify_seed */
    _rb_chno_ver = ver;
...
/*
 * bc_write_cco - bundle multiple compiled Compilers into one .cco file
 * (CHN 1.0 - Compiled CHN Object)
 */
BCResult bc_write_cco(const char *path,
                      ...
                      int count)
{
    /* We serialize each compiler as a function-only CHNO into a temp buffer,
     * then assemble the CCO container around all units. */

    /* Per-unit: raw CHNO bytes + source name */
    ...
    /* Track which unit has public entry main (first one found wins) */
    ...
    for(int u = 0; u < count; u++){
        ...
        /* Build CHNO blob: SECT_EXPORTS + SECT_FUNCS + SECT_INTERNAL [+ SECT_MAIN] */
        WBuf payload = {0};
        ...
        /* SECT_FUNCS - exported functions only (visible to outside importers) */
        sects[nsects] = wb_section_begin(SECT_FUNCS, &payload);
        wb_u32(&payload, (uint32_t)exported);
        for(int i = 0; i < C->func_count; i++)
            if(C->functions[i]->exported)
        ...
        /* SECT_INTERNAL - non-exported functions (1.0: bundle-internal linkage) */
        int internal_cnt = 0;
        for(int i = 0; i < C->func_count; i++)
        ...
        /* SECT_IMPORTS - external dependencies for the entry unit.
         * Stored so run_cco_file() can re-resolve them at runtime.
         * Written for ALL units so that nested CCO imports also resolve. */
        ...
        /* SECT_MAIN - entry body (1.0: only for the entry unit) */
        if(has_entry){
            sects[nsects] = wb_section_begin(SECT_MAIN, &payload);
            wb_codeobj(&payload, &C->top_chunk, NULL);
            wb_section_end(&sects[nsects++], &payload);
    ...
    /* v2 header: magic(4) + version(2) + flags(2) + unit_count(2) + entry_unit(2) = 12 bytes */
...
/*
 * cco_load_file - shared: read file into buffer, parse header.
 *   Returns allocated buffer (caller frees), fills header fields.
 *   Supports CCO v1 (8-byte) and v2 (12-byte) headers.
 */
typedef struct {
    uint8_t  *data;
    long      fsz;
    uint16_t  ver;
    uint16_t  flags;       /* 0 for v1 */
    uint16_t  unit_count;
    uint16_t  entry_unit;  /* CCO_NO_ENTRY for v1 or when no entry */
    size_t    units_start; /* byte offset where unit data begins    */
} CCOFile;
...
static BCResult cco_open(const char *path, CCOFile *out){
    ...
    if(ver == CCO_VERSION_LEGACY){
        /* v1: 8-byte header, no flags/entry */
                                 ...
                                 char ***seen_names, int *seen_count, int *seen_cap)
{
    ...
    /* Set CHNO version context so rb_codeobj handles minify_seed correctly */
...
/* These are registered into func_registry for the entry to call, but are     */
/* NOT returned to outside importers (bc_read_cco ignores this section).       */
    ...
    for(uint32_t si = 0; si < bnsects; si++){
        ...
        if(blob[de] == (uint8_t)SECT_INTERNAL){
            ...
            for(int i = 0; i < nf && !rb.err; i++){
                FunctionObject *fn = rb_codeobj(&rb, NULL);
                if(fn) func_register(fn); /* register but don't expose to callers */
            }
            return;
        }
    }
...
/* Used by bc_read_cco_entry_imports() to recover external dependencies.   */
...
/*
 * bc_read_cco_entry_imports - read SECT_IMPORTS from ALL CCO units.
 * Called by run_cco_file() before execution to resolve external deps.
 */
BCResult bc_read_cco_entry_imports(const char *path,
    ...
    for(int u = 0; u < (int)cf.unit_count; u++){
        ...
        /* Read embedded CHNO version for this unit */
        uint16_t unit_ver = 0;
        if(dlen >= 6)
...
/*
 * bc_read_cco - load all exported functions (v1 + v2 compatible)
 */
BCResult bc_read_cco(const char *path, FunctionObject ***out_funcs, int *out_nf){
    ...
    for(int u = 0; u < (int)cf.unit_count; u++){
        ...
        /* 1.0: also register internal funcs so exported funcs can call them at runtime */
...
/*
 * bc_read_cco_entry - load runnable CCO (v4.1)
 */
                            ...
                            Chunk *out_chunk, bool *out_has_entry)
{
    ...
    *out_has_entry = false;
    ...
    for(int u = 0; u < (int)cf.unit_count; u++){
        ...
        /* 1.0: Load SECT_INTERNAL first - bundle-internal functions must be
         * registered before exports or entry so cross-unit calls resolve */
        cco_load_unit_internal(blob, dlen);

        /* Load exports from every unit */
        cco_load_unit_funcs(blob, dlen, &all, &total, &cap, &seen, &seen_count, &seen_cap);

        /* Load entry chunk from the designated entry unit */
