...
static ASTNode *parse_stmt(Parser *P){
    ...
    if(check(P,TK_ELSE)){
        error_parse(P->current.line, P->current.col, P->current.length,
            NULL, "'else' must follow an 'if' block",
            "unexpected 'else' without a preceding 'if'");
        P->panic_mode=true; return NULL;
    }
    if(check(P,TK_CATCH)){
        error_parse(P->current.line, P->current.col, P->current.length,
            NULL, "'catch' must follow a 'try' block",
            "unexpected 'catch' without a preceding 'try'");
        P->panic_mode=true; return NULL;
    }
    if(check(P,TK_CASE)){
        error_parse(P->current.line, P->current.col, P->current.length,
            NULL, "'case' must be inside a 'switch' block",
            "unexpected 'case' outside 'switch'");
        P->panic_mode=true; return NULL;
    }
    if(check(P,TK_DEFAULT)&&!check2(P,TK_COLON)){
        error_parse(P->current.line, P->current.col, P->current.length,
            NULL, "'default:' must be inside a 'switch' block",
            "unexpected 'default' outside 'switch'");
        P->panic_mode=true; return NULL;
    }
    ...
    if(check(P,TK_THROW)){
        ...
        if(check(P,TK_NEWLINE)||check(P,TK_SEMICOLON)||check(P,TK_EOF)){
            error_parse(line, P->current.col, 1,
                "expression", "throw \"message\"  or  throw myError",
                "'throw' requires a value to throw");
            P->panic_mode=true; return NULL;
        }
...
static ASTNode *parse_native_call(Parser *P){
    ...
    if(!check(P,TK_NUMBER)){
        error_parse(P->current.line, P->current.col, P->current.length,
            "native call ID", "@native(42, arg1, arg2)",
            "expected a numeric native-call ID after '@native(', got %s",
...
static bool is_compound_assign(TokenKind k){
    return k==TK_PLUS_ASSIGN||k==TK_MINUS_ASSIGN||k==TK_STAR_ASSIGN||
           k==TK_SLASH_ASSIGN||k==TK_PERCENT_ASSIGN||k==TK_STARSTAR_ASSIGN; 
}
