...
static void skip_ws(Lexer *L){
    for(;;){
        ...
        switch(c){
            ...
            case '\n': return; /* col reset happens in adv() */
            case '-':
                if(pk2(L)=='-'){
                    while(!at_end(L)&&pk(L)!='\n') adv(L);
                } else return;
            ...
            case '/':
                if(pk2(L)=='/'){ 
                    ...
                    adv(L); adv(L); 
                    while(!at_end(L)){
                        if(pk(L)=='*'&&pk2(L)=='/'){ adv(L); adv(L); break; }
                        if(pk(L)=='\n'){ L->line++; L->col=1; }
                        adv(L);
                    }
                } else return;
...
static int process_escape(Lexer *L, char *buf, int pos){
    ...
    switch(e){
        ...
        case 'x':{ 
            char h1=adv(L),h2=adv(L);
            char hex[3]={h1,h2,'\0'};
            buf[pos++]=(char)strtol(hex,NULL,16);
            break;
        }
        case 'u':{ 
            char h[5]; for(int i=0;i<4;i++) h[i]=adv(L); h[4]='\0';
...
#define KW(str,_kk) if(len==(int)strlen(str)&&memcmp(s,str,len)==0){t.kind=(_kk);return t;}
    ...
    /* 'in' as expression operator when used as a keyword */
...
Token lexer_next(Lexer *L){
    ...
    switch(c){
        ...
        case '-':
            ...
            /* NOTE: '--' is CHN's line-comment marker (handled in skip_ws).
             * TK_MINUSMINUS is intentionally absent -- use '-= 1' to decrement. */
