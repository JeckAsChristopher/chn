...
#define PUSH(v) do{ \
    if(vm->stack_top>=MAX_STACK){RT_ERROR("stack overflow");} \
...
static void vm_throw_error(VM *vm, const char *msg){
    ...
    /* v3.3 fix: removed the dead first IP assignment
     * (ch->code + (tf->handler_ip - ch->code) == tf->handler_ip, then
     *  was immediately overwritten by the line below anyway). */
...
static void arr_cut(ObjArray *a, int i){
    /* safe bounds check - no crash on empty or out-of-range */
...
dispatch_switch:;
    ...
    CASE(OP_ADD):{
        ...
        if(IS_STRING(a)||IS_STRING(b)){
            ...
            if(!tmp) RT_ERROR("OOM in string concatenation");
    ...
    CASE(OP_NEG):{ Value a=POP();
        if(!IS_NUMBER(a)) RT_ERROR("unary '-' requires a number");
    ...
    CASE(OP_BNOT):{ Value a=POP();
        if(!IS_NUMBER(a)) RT_ERROR("'~' requires a number");
    ...
    CASE(OP_LT): { Value b=POP(),a=POP();
        ...
        else RT_ERROR("'<' requires numbers or strings");
        DISPATCH(); }
    ...
    CASE(OP_ARRAY_INDEX):{
        Value iv=POP(),av=POP();
        if(IS_DICT(av)){
            if(!IS_STRING(iv)) RT_ERROR("dict key must be a string, got %s",vtype(iv));
            PUSH(gc_dict_get(AS_DICT(av),AS_STRING(iv))); DISPATCH();
        }
        if(IS_STRING(av)){
            if(!IS_NUMBER(iv)) RT_ERROR("string index must be a number, got %s",vtype(iv));
        ...
        /* v3.6 safety: guard empty/null array before accessing items */
        if(!a->items || a->len==0) RT_ERROR("index %d out of bounds (array is empty)",(int)AS_NUMBER(iv));
        int idx=(int)AS_NUMBER(iv);
        if(idx<0) idx=a->len+idx;
        if(idx<0||idx>=a->len)
            RT_ERROR("index %d out of bounds (array length %d)",(int)AS_NUMBER(iv),a->len);
    ...
    CASE(OP_ARRAY_SET):{
        ...
        if(!IS_NUMBER(iv)) RT_ERROR("array index must be a number");
        ObjArray *a=AS_ARRAY(av);
        /* v3.6 safety: guard empty/null array */
        if(!a->items || a->len==0) RT_ERROR("index %d out of bounds (array is empty)",(int)AS_NUMBER(iv));
        int idx=(int)AS_NUMBER(iv);
        if(idx<0) idx=a->len+idx;
        if(idx<0||idx>=a->len)
    ...
    CASE(OP_ARRAY_LEN):{
        ...
        else RT_ERROR(".length() requires array, string, or dict, got %s",vtype(v));
        DISPATCH(); }
    ...
    CASE(OP_METHOD_CALL):{
        ...
        if(mid==METHOD_LENGTH){
            ...
            else RT_ERROR("method 'length' on %s",vtype(av));
        ...
        if(IS_STRING(av)){
            ...
            switch((ArrayMethod)mid){
                ...
                case METHOD_CONTAINS:{
                    if(argc<1||!IS_STRING(args[0])) RT_ERROR("contains: expects string arg");
        ...
        if(IS_DICT(av)){
            ...
            switch((ArrayMethod)mid){
                ...
                default: RT_ERROR("method '%s' not supported on dict",method_name((ArrayMethod)mid));
            }
        }

        if(!IS_ARRAY(av))
            RT_ERROR("method '%s' on %s (not array or string)",method_name((ArrayMethod)mid),vtype(av));
        ObjArray *a=AS_ARRAY(av);
        Value *args=&vm->stack[vm->stack_top-argc];
        switch((ArrayMethod)mid){
            case METHOD_ADD:    arr_push(a,args[0]); break;
            case METHOD_INSERT:{
                if(!IS_NUMBER(args[0])) RT_ERROR("insert: index must be number");
                int i=(int)AS_NUMBER(args[0]);
                if(i<0) i=a->len+i+1;
                arr_insert(a,i,args[1]); break;
            }
            case METHOD_CUT:{
                if(!IS_NUMBER(args[0])) RT_ERROR("cut: index must be number");
                int i=(int)AS_NUMBER(args[0]);
                if(i<0) i=a->len+i;
                if(i<0||i>=a->len) RT_ERROR("cut: index %d out of bounds",i);
                arr_cut(a,i); break;
            }
            case METHOD_REMOVE: arr_remove(a,args[0]); break;
            case METHOD_RALL:   arr_rall(a,args[0]);   break;
            case METHOD_SORT:
                /* v3.6 safety: only sort if items exist */
                if(a->items && a->len>1)
            ...
            case METHOD_JOIN:{
                ...
                for(int i=0;i<a->len;i++){
                    ...
                    else total+=8; /* rough estimate for numbers */
                }
            ...
            /* map/filter/reduce/any/all require calling a function value at runtime */
            case METHOD_MAP:{
                /* arr.map(fn) - fn is a compiled FunctionObject stored as Value */
                if(argc<1) RT_ERROR("map: expects a function argument");
                if(!IS_FUNCTION(args[0])) RT_ERROR("map: argument must be a function");
                /* map/filter/reduce/any/all: reserved for v4.4 (lambda support) */
    ...
    CASE(OP_DICT_SET):{
        ...
        if(!IS_DICT(dv)) RT_ERROR("DICT_SET on non-dict");
        if(!IS_STRING(key)) RT_ERROR("dict key must be a string");
        gc_dict_set(AS_DICT(dv),AS_STRING(key),val);
        PUSH(dv); DISPATCH(); }
    CASE(OP_DICT_GET):{
        Value key=POP(),dv=POP();
        if(!IS_DICT(dv)) RT_ERROR("DICT_GET on non-dict");
        if(!IS_STRING(key)) RT_ERROR("dict key must be a string");
    ...
    CASE(OP_FOREACH_INIT):{
        ...
        Value iterable=POP();
        if(!IS_ARRAY(iterable)) RT_ERROR("for-in requires an array, got %s",vtype(iterable));
        PUSH(iterable);
    ...
    CASE(OP_PUSH_HANDLER):{
        ...
        if(vm->try_top>=MAX_TRY_DEPTH) RT_ERROR("too many nested try blocks");
        Chunk *ch=FRAME.function?&FRAME.function->chunk:vm->top_chunk;
        TryFrame *tf=&vm->try_stack[vm->try_top++];
    ...
    CASE(OP_INC_LOCAL):{
        ...
        if(!IS_NUMBER(v)) RT_ERROR("INC_LOCAL: expected number, got %s",vtype(v));
    ...
    CASE(OP_DEC_LOCAL):{
        uint16_t s=READ_U16();
        Value v=get_local(vm,(int)s);
        if(!IS_NUMBER(v)) RT_ERROR("DEC_LOCAL: expected number, got %s",vtype(v));
        set_local(vm,(int)s,NUMBER_VAL(AS_NUMBER(v)-1.0));
        DISPATCH(); }

    CASE(OP_CALL_TAIL):{
        /* Tail-call optimisation: reuse current frame instead of pushing a new one.
         * Stack layout before:  [func][arg0][arg1]...[argN-1]
         * We overwrite the current frame's locals in-place and jump to the
         * callee's first instruction, avoiding any increase in frame_count.   */
        uint16_t argc=READ_U16();
        Value fv=vm->stack[vm->stack_top-argc-1];
        if(!IS_FUNCTION(fv)) RT_ERROR("attempt to tail-call a %s",vtype(fv));
        FunctionObject *f=AS_FUNCTION(fv);
        if((int)argc!=f->arity)
            RT_ERROR("'%s' expects %d arg(s) but got %d",f->name,f->arity,(int)argc);
        /* Copy arguments into the base of the current frame */
        int base=FRAME.base_idx;
    ...
    CASE(OP_AWAIT):
        /* identity - value stays on TOS unchanged */
        DISPATCH();

    /* OP_IN: val in container -> bool
     *   array:  linear scan for equality
     *   dict:   key membership check (key must be string)
    ...
    CASE(OP_IN):{
        ...
        if(IS_ARRAY(container)){
            ...
            if(IS_STRING(val)){
                ...
                RT_ERROR("'in' on dict requires a string key, got %s",vtype(val));
            }
        } else if(IS_STRING(container)){
            if(!IS_STRING(val)) RT_ERROR("'in' on string requires a string needle, got %s",vtype(val));
            ...
            RT_ERROR("'in' requires array, dict, or string on right side, got %s",vtype(container));
        }
